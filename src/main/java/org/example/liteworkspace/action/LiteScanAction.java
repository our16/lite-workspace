...
public class LiteScanAction extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        final PsiMethod[] targetMethod = new PsiMethod[1];
        ...
        try {
            ReadActionUtil.runSync(project, () -> {
                ...
                targetClass[0] = classes[0];
                if (targetClass[0] == null) {
                    LogUtil.warn("目标类为空");
                    return;
                }
                
                LogUtil.info("找到目标类: {}", targetClass[0].getQualifiedName());
                
                // 将目标类转换为DTO，但不长期保存PSI对象
                ClassSignatureDTO targetClassDto = PsiToDtoConverter.convertToClassSignature(targetClass[0]);
                LogUtil.debug("目标类转换为DTO: {}", targetClassDto);
                
                // 获取目标方法
                CostUtil.start("getTargetMethod");
                targetMethod[0] = getTargetMethod(e);
                CostUtil.end("getTargetMethod", "获取目标方法");
                
                if (targetMethod[0] == null) {
                    LogUtil.info("没有找到具体方法");
                } else {
                    LogUtil.info("找到目标方法: {}", targetMethod[0].getName());
                    // 将目标方法转换为DTO，但不长期保存PSI对象
                    MethodSignatureDTO targetMethodDto = PsiToDtoConverter.convertToMethodSignature(targetMethod[0]);
        ...
        new Task.Backgroundable(project, "LiteWorkspace 生成中...", true) {
            ...
            public void run(ProgressIndicator indicator) {
                try {
                    ...
                    service.scanAndGenerate(targetClass[0], targetMethod[0], indicator);
